...
def get_whole_sign_houses(
    ...
    House 1 = 0° of Ascendant's sign
    Each house = one full zodiac sign (30°)
    """
    import pytz
    tz = pytz.timezone(timezone_name)
    ...
    # Get Ascendant and MC using any quadrant system (we just need ASC degree)
    cusps, ascmc = swe.houses(jd, lat, lon, b'E')
    asc_lon = ascmc[0]
    mc_lon = ascmc[1]

    # Whole Sign: Ascendant sign = House 1
    asc_sign_index = int(asc_lon / 30) % 12

    houses = []
    for i in range(12):
        house_sign_index = (asc_sign_index + i) % 12
        house_cusp = house_sign_index * 30  # 0° of each sign
        houses.append({
            'house': i + 1,
            'sign': SIGNS[house_sign_index],
            'symbol': GLYPHS[house_sign_index],
            'longitude': round(house_cusp, 4),
            'degree': 0.0,
        })
    ...
    return {
        'ascendant': _sign_info(asc_lon),
        'midheaven': _sign_info(mc_lon),
        'vertex': _sign_info(ascmc[3]) if len(ascmc) > 3 else _sign_info(0),
        'houses': houses,
        'system': 'W',  # Whole Sign
        'julian_day': jd,
        'location': {'lat': lat, 'lon': lon},
